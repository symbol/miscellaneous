...
class XYMStateMap():
    ...
    harvester_mosaics: list[str]
        List of string alias(es) for harvester mosaic

    """

    def __init__(self, state_map=None, account_map=None, mosaic_map=None):
        ...
        if len(state_map):
            state_map = {key: {
                ...
                'mosaics': defaultdict(list, val['mosaics']),
        ...
        # add invertible account map?
        self._account_map = account_map

        self._mosiac_map = mosaic_map

        self._height_ts_map = {}

        self.harvester_mosaics = ['0x6bed913fa20223f8', '0xe74b99ba41f4afee']  # only care about XYM for now, hardcoded alias
    ...
    def read_msgpack(cls, msgpack_path):
        ...
        if isinstance(msgpack_path, str):
            with open(msgpack_path, 'rb') as file:
                state_map, account_map, mosaic_map = msgpack.unpack(file, unicode_errors=None, raw=False)
        else:
            raise TypeError(f'Unrecognized type {type(msgpack_path)} for read_msgpack, path str')

        return cls(state_map=state_map, account_map=account_map, mosaic_map=mosaic_map)
    ...
    def to_msgpack(self, msgpack_path):
        ...
        with open(msgpack_path, 'wb') as file:
            file.write(msgpack.packb((self.to_dict(), self._account_map, self._mosiac_map)))
    ...
    def insert_txn(self, txn, height, fee_multiplier):
        ...
        # need to handle flows for *all* mosaics, not just XYM
        ...
        if txn['type'] == b'4154':  # transfer txn
            if len(txn['payload']['message']) and txn['payload']['message'][0] == 0xfe:
                ...
                for mosaic in txn['payload']['mosaics']:
                    if hex(mosaic['mosaic_id']) in self.harvester_mosaics:
                        ...
                        self._state_map[address]['mosaics'][hex(mosaic['mosaic_id'])].append((height, -mosaic['amount']))
                        self._state_map[txn['payload']['recipient_address']]['mosaics'][hex(mosaic['mosaic_id'])].append(
        ...
        elif txn['type'] in [b'434d']:
            if hex(txn['payload']['mosaic_id']) in self.harvester_mosaics:
                self._state_map[txn['payload']['source_address']]['xym_balance'][height] -= txn['payload']['amount']

        elif txn['type'] == b'0x434e':  # mosaic alias
            if txn['payload']['alias_action'] == 0x1:
                self._mosiac_map[txn['payload']['namespace_id']] = txn['payload']['mosaic_id']
            else:
                self._mosiac_map[txn['payload']['namespace_id']] = None

        # need to add address alias resolution
    ...
    def get_harvester_graph(self, height=np.inf, min_harvester_size=10000, min_node_size=10000, track_remote=False):
        ...
        for key, val in self._state_map.items():
            ...
            if balance >= min_harvester_size:
                ...
                num_h = sum(map(partial(lambda s, h, x: int(s <= x <= h), link_start, height), val['harvested'].keys()))
